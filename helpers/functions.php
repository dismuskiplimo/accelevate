<?php
	...
	function profile_thumb($asset = ''){
		$path = base_path() . '/' . env('PUBLIC_PATH') . '/' . 'img/uploads/thumbnails/';
	...
	function cover($asset = ''){
		$path = base_path() . '/' . env('PUBLIC_PATH') . '/' . 'img/uploads/covers/';

		$file = $path . $asset;

		if($asset){
			if(file_exists($file)){
				return my_asset('img/uploads/covers/' . $asset);
			}else{
				return my_asset('img/cover.png');	
			}
		}else{
			return my_asset('img/cover.png');
		}
